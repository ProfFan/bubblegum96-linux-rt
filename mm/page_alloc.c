...
#ifdef CONFIG_CMA
void adjust_alloc_cma_page_count(struct zone *zone)
{

	long total, cma, movable;

	total = global_page_state(NR_FREE_PAGES);
	cma = global_page_state(NR_FREE_CMA_PAGES);
...
#ifdef CONFIG_CMA
static struct page *__rmqueue_cma(struct zone *zone, unsigned int order, int migratetype)
{
	struct page *page;
	...
	if (zone->nr_try_cma > 0  &&   zone->managed_cma_pages) {
	...
	if (zone->nr_try_movable > 0)
		goto alloc_movable;
	adjust_alloc_cma_page_count(zone);
...
extern void boost_cma_requests_start(unsigned long start_pfn,
				unsigned long end_pfn);
